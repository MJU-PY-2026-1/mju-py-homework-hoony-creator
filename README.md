[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/MiVO5zfB)
# 🐍 [2026-01] 나만의 파이썬 소프트웨어 개발 프로젝트

## 1. 시나리오 제목
* 프로젝트 볼트(Project Vault): 장기 프로젝트·포트폴리오 통합 관리 시스템
<br>


## 2. 시나리오 (5~10줄)
* 이 프로그램은 사용자가 진행 중인 장기 프로젝트를 기록하고, 그 과정에서 생기는 링크, 파일, 메모, 결과물을 함께 정리할 수 있는 개인용 관리 소프트웨어입니다.
* 가장 먼저 사용자는 프로젝트 이름, 프로젝트 분야, 최종 목표, 현재 진행률, 중요도, 보유한 자료 개수 같은 기본 정보를 입력하여 하나의 프로젝트 카드를 생성합니다.
시스템은 입력된 값들을 바탕으로 해당 프로젝트의 초기 준비도와 우선순위를 계산하여, 지금 당장 관리가 필요한 프로젝트가 무엇인지 보여줍니다.
이후 사용자는 프로젝트와 관련된 발표 자료 링크, 코드 저장소 주소, 참고 문서, 회고 메모 등을 계속 추가하여 하나의 포트폴리오처럼 정리할 수 있습니다.
프로젝트가 진행될수록 수행 과정과 결과물이 함께 누적되기 때문에, 단순히 할 일을 적는 것이 아니라 내가 무엇을 해왔고 무엇이 부족한지를 한눈에 파악할 수 있습니다.
나중에는 여러 개의 프로젝트를 리스트로 저장하고, 조건문을 이용해 프로젝트 상태를 ‘순항’, ‘정체’, ‘위험’ 등으로 판정할 예정입니다.
최종적으로는 사용자가 메뉴를 통해 프로젝트 등록, 자료 추가, 상태 확인, 전체 조회, 포트폴리오 요약 출력까지 할 수 있는 실용적인 통합 관리 소프트웨어로 완성할 계획입니다.

<br>


## 3. 예상 기능 및 메뉴 (최소 5개)
1. 프로젝트 등록
2. 초기 준비도 점수 계산
3. 프로젝트 우선순위 확인
4. 링크·파일·메모 자료 등록
5. 프로젝트 상태 확인
6. 전체 프로젝트 정보 출력
7. 포트폴리오 요약 출력
8. 프로그램 종료

<br>

# 🚀 [버전별 개발 일지 & AI 협업 기록]

## 🟦 [1차 과제: V1.0] 시나리오 기획
    
### **🤖 AI 파트너십 과정**
(예시)
 1. **내용 1 : 컨셉 통합 및 주제 확정**
    * **프롬프트 요약:** ""장기 프로젝트 진행도 관리기와 포트폴리오 관리기 중 실사용성이 높은 방향을 비교하고, 필요하면 두 아이디어를 하나의 소프트웨어 컨셉으로 통합해줘.""
    * **적용 내용:** AI와 논의한 결과, ‘진행 관리’와 ‘결과물 저장’을 분리하는 것보다 하나의 시스템으로 합치는 것이 실제 활용도가 높다고 판단하였다. 그래서 단순한 할 일 관리 프로그램이 아니라, 장기 프로젝트의 진행 상황과 포트폴리오 자료를 함께 관리하는 ‘프로젝트 볼트’ 컨셉을 최종 주제로 채택하였다.
      
 2. **내용 2 : 단계별 확장 로드맵 및 1차 입력 구조 설계**
    * **프롬프트 요약:** “성적 관리 프로그램처럼 1차는 입출력/연산, 2차는 리스트/조건문, 3차는 메뉴/반복문, 4차는 함수/딕셔너리 구조로 발전할 수 있도록 프로젝트 볼트의 기능 로드맵과 1차 입력 데이터 구조를 설계해줘.”
    * **적용 내용:** AI가 제안한 로드맵을 바탕으로, 1차에서는 프로젝트명(str), 분야(str), 현재 진행률(int), 자료 개수(int), 중요도(float) 등을 입력받아 기초 준비도 점수를 계산하는 구조를 설계하였다. 또한 2차 이후에는 여러 프로젝트를 리스트에 저장하고, 조건문으로 상태를 분석하며, 3차와 4차에는 메뉴 시스템·함수 분리·딕셔너리 기반 프로젝트 카드 관리로 확장하는 방향을 확정하였다.
    
### **📁 증빙 자료:**
  * [1차_AI협업캡처.pdf 첨부 완료] (첨부 후 링크)

<br>

## 🟩 [2차 과제: V1.0] 입출력 + 리스트 + 조건문 - 향후 작성 예정
### **✨2차 과제 내용:**
(예시)
  * 이번 과제는 용사 등록 및 정밀 진단 시스템 구현으로, 사용자로부터 데이터를 입력받아 리스트에 저장하고, 가중치 연산을 통해 결과값을 도출하는 시스템을 구축하는 것입니다. 특히 `if-elif` 조건문과 `and/or` 논리 연산자를 활용하여 상황에 맞는 등급과 특별 칭호를 부여하는 지능형 로직을 완성해야 합니다.
  * input()으로 용사의 이름, 체력, 마력, 공격력을 입력받아 리스트에 저장.
  * 가중치 공식을 적용하여 종합 전투력 산출: (체력 * 0.7) + (마력 * 0.5) + (공격력 * 1.8).
  * if-elif-else를 사용하여 S등급부터 F등급까지 5단계 등급 판정 로직 추가.
  * and 연산자를 활용하여 '전설의 대마법사' 등 특별 칭호 부여 기능 구현.
    
### **🤖 AI 파트너십 과정**
(예시) 
1. **내용 1: 길드 매니저 컨셉 및 로직 도출**
    * **프롬프트 요약:** "내가 길드 매니저가 되어 용사를 관리하는 프로그램을 만들 거야. 1학년 수준에서 리스트와 조건문을 활용해 등급을 매기는 재미있는 로직을 짜줘."
    * **적용 내용:** AI가 제안한 '가중치 전투력 산출' 방식과 'S등급이면서 마력이 높으면 대마법사 칭호 부여'라는 복합 조건 아이디어를 실제 코드에 반영함.

2. **내용 2: 효율적인 리스트 활용법 자문**
    * **프롬프트 요약:** "파이썬 기초 문법 단계(리스트 → 조건문 → 반복문 → 함수/딕셔너리)에 맞춰, '용사 길드 관리자' 컨셉의 프로그램이 어떻게 발전할 수 있을지 4단계 기능 확장 로드맵을 제안해줘."
    * **적용 내용:** 단순 입출력을 넘어, **'기초 정보 입력 및 리스트 저장(1차) → 전투 등급 판독기 구현(2차) → 무한 메뉴 시스템 구축(3차) → 딕셔너리를 활용한 다중 캐릭터 관리(4차)'**로 이어지는 논리적인 확장 로드맵을 확립함.
    
### **🛠️ Troubleshooting & 기술 회고:**
(예시) 
1. **문제 1: 숫자를 입력했는데 계산이 안 되는 현상 (TypeError)**
    * **원인:** `input()`으로 받은 데이터는 기본적으로 '글자'라서 숫자와 곱셈 연산이 불가능했음.
    * **해결:** `int()`와 `float()` 함수를 사용하여 입력값을 숫자로 바꾸는 '형변환' 마법을 부려 해결함.
2. **문제 2: 조건문 뒤에 자꾸 생기는 빨간 줄 (SyntaxError)**
    * **원인:** `if`와 `elif` 조건식 끝에 마법의 기호인 `:`(콜론)을 자꾸 빠뜨려서 발생함.
    * **해결:** 파이썬의 모든 조건문과 반복문 끝에는 항상 땡땡(`:`)이 들어가야 한다는 규칙을 학습하고 수정함.
     
### **📁 증빙 자료:**
  * [2차_AI협업캡처.pdf 첨부 완료] (첨부 후 링크)
  * [2차과제_실행결과.jpg]
<br>

## 🟨 [3차 과제: V3.0] 무한 루프와 메뉴 시스템 (반복문) - 향후 작성 예정
### **✨3차 과제 업데이트 내용:**
  * 내용.
    
### **🤖 AI 파트너십 과정**
 1. **내용 1**
    * **프롬프트 요약:**  ... 
    * **적용 내용:** ....
    
### **🛠️ Troubleshooting & 기술 회고:**
  1. **문제 1:** ...
     * **원인:** ...
     * **해결:** ..
     
### **📁 증빙 자료:**
  * [3차_AI협업캡처.pdf 첨부 완료] (첨부 후 링크)
  * [3차과제_실행결과.jpg]
<br>

### 🟥 [4차 과제: V4.0] 모듈화 및 데이터 확장 (배열과 함수) - 🌟최종 완성 -- 향후 작성 예정
### **✨4차 과제 업데이트 내용:**
  * 내용.
    
### **🤖 AI 파트너십 과정**
 1. **내용 1**
    * **프롬프트 요약:**  ... 
    * **적용 내용:** ....
    
### **🛠️ Troubleshooting & 기술 회고:**
  1. **문제 1:** ...
     * **원인:** ...
     * **해결:** ..
     
### **📁 증빙 자료:**
  * [4차_AI협업캡처.pdf 첨부 완료] (첨부 후 링크)
  * [4차과제_실행결과.jpg]
<br>
